...
Al ejecutar `base-wide.R` se crear una base de datos ancha, con los proyecto de investigación como unidad de análisis. Esta base se exporta a `app/data/base_proyectos.rdata` y se utiliza en la App de Consultas Internas de CINDAI.

### Anonimización

`anonimizacion.R` es un script que permite anonimizar la base de datos integrada, eliminando variables sensibles. Más detalles sobre este proceso se revisaron en el @sec-x
